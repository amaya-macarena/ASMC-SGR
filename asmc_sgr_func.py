# -*- coding: utf-8 -*-
"""
Adaptive Sequential Monte Carlo inversion combined with Sequential Geostatistical Resampling for posterior inference and evidence computation. This code contains auxiliary functions called by asmc_sgr.py.
...
#                log_p[ii,0]= - 0.5 * np.power(Sigma[ii],-2.0) * np.sum( np.power(e,2.0) )
...
def Metrop(MCMCPar,log_p_xnew,log_p_xold,Extra,beta):    
...
#def Dreamzs_finalize(MCMCPar,Z,Sequences,outDiag,fx,iteration,iloc,pCR,m_z,m_func):
...
#		print('found',found)
		...
		if (mid==0): #and not mid==len(beta_list)-1):  #Lower bound(mid==0) , apply smaller step')
			...
			beta_found=beta+inc_found#betainc_list[0]
#			inc_found=10**(betaexp_list[0])#betainc_list[0]        
...
#       
		if (mid==len(gamma_list)-1): #		Upper bound
			...
			inc_found=10**(omega_new)
#			inc_found=10**(-5+betaexp_list[len(betaexp_list)-1])#betainc_list[len(betainc_list)-1]
			beta_found=beta+inc_found#betainc_list[len(betainc_list)-1]
			contribution_found=np.exp((beta_found-beta)*curr_log_lik)
			CESS_found=seq * (np.sum(norm_weight*contribution_found))**2 / np.sum(norm_weight*contribution_found**2)		
...
#		print('curr_log_lik',curr_log_lik)       
...
#	print('CESS_found',CESS_found)    
#	print('inc_found',inc_found)  
#	print('gamma_list[mid]',gamma_list[mid])
#	print('omega',omega)
...
def ds_parallel(njobs,u,T):
    

    result=Parallel(n_jobs=njobs)( delayed(runDS_con)(i,T) for i in u)
    all_res=np.vstack(result)
    
    return all_res


    
def start_engine(n,u):
    result=[];
    for i in range(n):
       result.append(matlab.engine.start_matlab());
...
def run_forward_par(njobs,model,u,eng,start_engine_paralel):
    ...
    wells=np.array([2,9,16,23,30,37,44,51,58,65,72])
    idat=11
    d=np.zeros((T,30,idat))
    u=u-1
    
    if start_engine_paralel=='No':
        result=Parallel(n_jobs=njobs)(delayed(run_forward)(K[i,:,:],wells,idat,eng,start_engine_paralel) for i in u)
    
    if start_engine_paralel=='Yes':
        result = Parallel(n_jobs=njobs,prefer="threads")( delayed(run_forward)(K[i,:,:],wells,idat,eng[i],start_engine_paralel) for i in u )
        
    all_res=np.vstack(result)
   
    return all_res
    
def run_forward(K,wells,idat,eng,start_engine_paralel):
    
    if start_engine_paralel=='No':
        eng = matlab.engine.start_matlab()
        
    A=matlab.double(K.tolist())
    
    conc=eng.MaFloT(A) 
    conc_np=np.array(conc)
    btc=np.zeros((1,30,idat))

    for j in range(idat):
        btc[0,:,j]=conc_np[wells[j],50,:]               
    return btc
